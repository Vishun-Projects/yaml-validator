...
data = read_yaml_file('win_data.yaml')
fields = data['fields']
rows = data['rows']
...
# Ensure rows are initialized to accommodate names from names YAML file
...
fields = ['Tester', 'Device & Model'] + [category['category'] for category in categories]
...
filenames = [f"VQE_Assignments{date.strftime('%Y-%m-%d')}.csv" for date in weekdays]
